...
# Handle PEP 668 (externally-managed-environment) on Arch, Debian, etc.
PIP_FLAGS="--user --quiet"
$PIP install $PIP_FLAGS requests 2>/dev/null || \
$PIP install --user requests 2>/dev/null || \
$PIP install --user --break-system-packages requests 2>/dev/null || \
$PIP install --break-system-packages requests 2>/dev/null || {
  echo -e "${ORANGE}Could not auto-install requests. Trying pipx...${NC}"
  pipx install requests 2>/dev/null || true
}

if [ $HAVE_FUSE -eq 1 ]; then
    echo -e "${GREEN}✓ FUSE detected — mount tool will be available${NC}"
    $PIP install $PIP_FLAGS fusepy 2>/dev/null || \
    $PIP install --user --break-system-packages fusepy 2>/dev/null || \
    pipx install fusepy 2>/dev/null || true
else
